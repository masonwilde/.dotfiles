...
- Always keep a 5 minute stall monitor running while any subagent is in flight. Arm a Monitor (or a scheduled wakeup) that reports every 5 minutes what each running agent's worktree shows: HEAD, the size of the uncommitted diff in lines, the time of the newest source edit, and whether a cargo or rustc process is alive. A count of changed files is not enough, since an agent reworking the same files keeps it flat. Two consecutive reports with no change in any of those means the agent has stalled: send it a wrap-up message, and if the next report still shows no change, stop it and relaunch with a tighter brief. Never let a subagent run unwatched.
