...
- Always keep a 10 minute stall monitor running while any subagent is in flight. Arm a Monitor (or a scheduled wakeup) that reports every 10 minutes what each running agent's worktree shows: HEAD, uncommitted file count, and whether a cargo or rustc process is alive. Two consecutive reports with no change means the agent has stalled: send it a wrap-up message, and if the next report still shows no change, stop it and relaunch with a tighter brief. Never let a subagent run unwatched.
