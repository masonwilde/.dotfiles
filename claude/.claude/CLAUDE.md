...
- Every subagent brief tells the agent that its tool call descriptions are its live status line for the user, so each one names the step and the file in plain words, never a generic "debugging" or "running command".
- Every subagent brief states a start condition (the state it begins from and how to verify it) and an end condition (the exact deliverable that means it is done), plus a budget and a "stop and report if" clause for foreseeable blockers.
- ALWAYS end a subagent and start a new one for each discrete task. When an agent reports, its job is over: stop it (TaskStop) so its timer ends, and launch a fresh agent with a fresh brief for the next task, including fixes to that agent's own work. Never resume a finished agent, and message a running one only to tell it to wrap up.
- Always keep a 10 minute stall monitor running while any subagent is in flight. Arm a Monitor (or a scheduled wakeup) that reports every 10 minutes what each running agent's worktree shows: HEAD, the size of the uncommitted diff in lines, the time of the newest source edit, and whether a cargo or rustc process is alive. A count of changed files is not enough, since an agent reworking the same files keeps it flat. Two consecutive reports with no change in any of those means the agent has stalled: send it a wrap-up message, and if the next report still shows no change, stop it and relaunch with a tighter brief. Never let a subagent run unwatched.
